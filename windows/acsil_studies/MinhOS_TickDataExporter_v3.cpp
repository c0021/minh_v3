/*
MinhOS Tick Data Exporter & Trade Executor v3 - PHASE 3 MICROSECOND TICK DATA - FINAL
=====================================================================================
...
SCDLLName("MinhOS Tick Data Exporter v3 - Production Ready")
...
static float g_CumulativeVolume = 0.0f;  // Track cumulative volume manually
...
SCSFExport scsf_MinhOSTickExporter_v3(SCStudyInterfaceRef sc)
{
    ...
    {
        // Set study defaults
        sc.GraphName = "MinhOS Tick Data Exporter v3 - Final";
        sc.StudyDescription = "Ultra-high precision tick-by-tick market data export with microsecond timestamps - Production Ready";
        ...
        Subgraph_Output.PrimaryColor = RGB(0, 255, 255);  // Cyan for v3
        ...
        Input_SymbolFilter.SetString("NQ,ES,VIX");
    ...
    if (Input_EnableLogging.GetYesNo()) {
        sc.AddMessageToLog("MinhOS v3 Final: Processing tick data", 0);
    }
...
void ExportTickData(SCStudyInterfaceRef sc, const SCString& OutputPath)
{
    ...
    // Track cumulative volume manually
    g_CumulativeVolume += Volume;
    ...
    if (sc.Bid > 0.0f && sc.Ask > 0.0f) {
        BidPrice = sc.Bid;
        AskPrice = sc.Ask;
        BidSize = (uint32_t)(Volume * 0.4f);  // Estimated bid-side volume
        AskSize = (uint32_t)(Volume * 0.6f);  // Estimated ask-side volume
    } else {
        ...
        BidSize = (uint32_t)(Volume / 2);
        AskSize = (uint32_t)(Volume / 2);
    }
    
    // Detect new trades with enhanced logic
    bool isNewTrade = (CurrentPrice != g_LastPrice) || (Volume > 0);
    if (isNewTrade || g_LastPrice == 0.0f) {
        // Create enhanced tick trade record
        TickTrade tick;
        tick.timestamp_us = g_Timer.GetUnixMicroseconds();
        tick.price = CurrentPrice;
        tick.size = (uint32_t)Volume;
        tick.bid = BidPrice;
        tick.ask = AskPrice;
        ...
        tick.high = High;
        tick.low = Low;
        tick.total_volume = (uint32_t)g_CumulativeVolume;  // Use our tracked volume
...
void WriteHighFrequencyTick(const SCString& outputPath, const SCString& symbol, const TickTrade& tick, bool isBatch)
{
    ...
    json << "  \"source\": \"sierra_chart_acsil_v3_final\",\n";
...
void ExecuteTrade(SCStudyInterfaceRef sc, const SCString& symbol, const SCString& side, int quantity, float price, const SCString& orderType)
{
    ...
    SCString logMsg;
    logMsg.Format("MinhOS v3 Final: Executing %s %d %s at %.6f (timestamp: %llu μs)", 
...
void LogPerformanceMetrics(SCStudyInterfaceRef sc)
{
    ...
    SCString perfMsg;
    perfMsg.Format("MinhOS v3 Performance: Trades=%u, Buffer=%u/%u, Uptime=%llu μs", 
                   g_TotalTrades, 
